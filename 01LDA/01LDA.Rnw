...
\begin{itemize}
  ...
  \item $b_i \sim \N(0,D)$: subject-specific ``\underline{random effects}'' for subject $i$\\
  \item $(\varepsilon_{i1},\ldots,\varepsilon_{i4}) \sim \N(0,\Sigma_i)$: vector of ``\underline{residuals}'' for subject $i$\\
  \item $D,\, \Sigma_i$: ``\underline{variance components}''\\
\end{itemize}
...
lme(ADAS11 ~ age + female + month + month:active, 
  data = trial_obs, random = ~month|id)
@
\end{frame}
